...
function duplicateInArray(obj, n) {
  // Creates multiple decks in an array
  const copies = [];
  for (let i = 0; i < n; i++) {
...
function playerTurn(active = true) {
  // Enables/disables player buttons.
  if (active) {
...
function clearTable() {
  // Resets game for consecutive games
...
function dealCard(hand, n = 1) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let drawnCard =
      Object.keys(drawnDeck)[getRandomInt(Object.keys(drawnDeck).length)];
...
function scoreHand(hand) {
  // Calculates score (including ace value).
...
function displayCard(hand, holeCard = false) {
  // Display cards on screen.
  handString = hand == dealer ? "#dealerRow" : "#playerRow";
  let location = document.querySelector(handString);
  cardString =
    holeCard == false
      ? Object.keys(hand)[Object.keys(hand).length - 1]
      : "back";
  location.insertAdjacentHTML(
    "beforeend",
    `<div class="card" id="${cardString}"> <img src="./deck/${cardString}.png" /></div>`
  ); //
}

function displayHoleCard() {
  // Reveal hole card on screen.
